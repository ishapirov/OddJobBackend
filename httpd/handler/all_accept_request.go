...
func AllAcceptRequestGet(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {

		userID := c.DefaultQuery("user", "-1")

		query := "SELECT * FROM requests WHERE accept_id = '" + userID + "';"
...
type requestArr2 struct {
	RequestArray []request.Request `json:"requestArray"`
}
